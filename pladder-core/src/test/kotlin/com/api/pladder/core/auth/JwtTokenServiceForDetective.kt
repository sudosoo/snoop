...
import com.api.pladder.TestData.testDetective
...
@SpringBootTest(classes = [JwtTokenServiceForDetective::class])
@ActiveProfiles("local")
class JwtTokenServiceForDetective(
    @Value("\${jwt.secret}")
    ...
    Given("Detective token 테스트") {
        When("토큰 생성 요청을 하면") {
            val token = service.generate(testDetective)
            Then("토큰이 생성 된다") {
                print(token)
            }
        }
        ...
        When("유효한 토큰을 넣으면") {
            ...
            Then("사용자 정보가 추출된다") {
                admin.userId shouldBe(testDetective.userId)
            }
        }
        When("조작된 토큰을 넣으면") {
            val invalidToken = "invalidToken"
